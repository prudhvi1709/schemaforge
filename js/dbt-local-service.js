...
/**
 * Builds the DBT project zip and returns a Blob without downloading it.
 * Used by the cloud run feature to upload to the remote sandbox.
 */
export async function buildDbtZip(schemaData, dbtRulesData, fileData) {
  if (typeof JSZip === 'undefined') {
  ...
  const zip = new JSZip();
  const datasetName = fileData.name
    .replace(/\.(csv|xlsx?)$/i, '')
    .replace(/^dataset-/, '')
